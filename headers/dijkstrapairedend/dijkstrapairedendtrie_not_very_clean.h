        ...
        if (w.lpos==-1 && w.rpos==-1){
                ...
                /*for (int i = 0; i < 4; ++i)
                    for (int j = 0 ; j < 4; ++j){
                        ...
                        if (w.v->child[j] != nullptr){
                            ...
                            nb = Nodeptr(w.qpos, -1, -1, w.u, w.v->child[j], w.cost);
                            q.push(nb);
                        }
            ...
            if (leafr){
                for (int j = last[w.v->num]; j != -1; j = prevpos[j]){
                    ...
                    nb = Nodeptr(w.qpos, -1, j+1, w.u, nullptr, w.cost);
                    q.push(nb);
                }
                continue;
            }
            ...
            //nb.print();
            //bool optimizationcost = false;
            for (int i = 0; i < 4; ++i){
                if (w.u->child[i] == nullptr)
                    continue;
                //leafl = false;
                //if(optimizationcost == true)
                for (int j = 0; j < 4; ++j){
                    if (w.v->child[j] == nullptr)
                        continue;
                    /*if(optimizationcost == true)
                    ...
                    nb = Nodeptr(w.qpos, -1, -1, w.u, w.v->child[j], w.cost+1);
                    q.push(nb);
                    assert(cout<<"Third pushed: ");
                    //nb.print();

                    //nb = Nodeptr(w.qpos+1, w.u, w.v->child[j], w.cost+1);
                    nb = Nodeptr(w.qpos+1, -1, -1, w.u, w.v->child[j], w.cost+1);
                    if (right[w.qpos] != base[j])
                        ++nb.cost;
                    q.push(nb);
