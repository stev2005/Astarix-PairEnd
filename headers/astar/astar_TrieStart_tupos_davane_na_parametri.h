...
State PushState(State cur, int k, const vector<int> &seeds, const vector<int> &last){
    return State(cur.qpos, cur.p, cur.cost+1, seed_heuristic(cur, k, seeds, last));
}
...
vector<State> getNextStates(State cur, char c, int k,const string &ref, const vector<int> &seeds, const vector<int> &last, const vector<int> &prevpos){
    ...
    if (cur.p.is_in_trie()){
        ...
        inheritors.push_back(PushState())
        for (int i = 0; i < 4; ++i)
            if (cur.p.u->child[i] != nullptr){
                inheritors.push_back(State(cur.qpos+1, Node(cur.p.u->child[i]), cur.cost, seed_heuristic(State(cur.qpos+1, Node(cur.p.u->child[i])), k, seeds, last)));
                if (base[i] != c)inheritors.back().cost++;
                inheritors.push_back(State(cur.qpos, Node(cur.p.u->child[i]), cur.cost+1, seed_heuristic(State(cur.qpos, Node(cur.p.u->child[i])), k, seeds, last)));
            }
    }
    else{
