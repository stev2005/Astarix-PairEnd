...
testchep_dijkstra_heuristic:
	time ./main.exe --alignment paired-end --heuristic dijkstra_heuristic --trie-depth 10 --seed-len 12 --insert-distance 5000 --filter-distance-difference 500 --reference $(PairedEndDIR)/reference.in --query $(PairedEndDIR)/1.in >paired-end.out

testpairend_seed_heuristic:
	time ./main.exe --alignment paired-end --heuristic seed_heuristic --trie-depth 10 --seed-len 12 --insert-distance 5000 --filter-distance-difference 500 --reference $(PairedEndDIR)/reference.in --query $(PairedEndDIR)/1.in >paired-end.out
